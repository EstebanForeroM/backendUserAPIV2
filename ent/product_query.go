...
func (pq *ProductQuery) QueryOrders() *OrderQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, product.OrdersTable, product.OrdersColumn),
		)
...
func (pq *ProductQuery) QueryCarts() *CartQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, product.CartsTable, product.CartsColumn),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if pq.withOrders != nil || pq.withCarts != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withOrders; query != nil {
		if err := pq.loadOrders(ctx, query, nodes, nil,
			func(n *Product, e *Order) { n.Edges.Orders = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withCarts; query != nil {
		if err := pq.loadCarts(ctx, query, nodes, nil,
			func(n *Product, e *Cart) { n.Edges.Carts = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProductQuery) loadCarts(ctx context.Context, query *CartQuery, nodes []*Product, init func(*Product), assign func(*Product, *Cart)) error {
	...
	nodeids := make(map[uuid.UUID][]*Product)
	for i := range nodes {
		if nodes[i].cart_products == nil {
			continue
		}
		fk := *nodes[i].cart_products
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "cart_products" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
